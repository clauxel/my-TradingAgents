...
function resolveLocalPostgresConfig({
  ...
  const password = firstDefined(
    ...
    readDpapiSecretFile(resolveProjectPath(projectRoot, secretFile)),
...
CONNECTION_URL="\${MULTICA_POSTGRES_URL:-\${DATABASE_URL:-\${POSTGRES_URL:-\${POSTGRES_PRISMA_URL:-}}}}"
...
if [ -z "$PGHOST_VALUE" ] || [ -z "$PGDATABASE_VALUE" ] || [ -z "$PGUSER_VALUE" ] || [ -z "$PGPASSWORD_VALUE" ]; then
  echo "PostgreSQL variables are missing from $ENV_FILE. Set MULTICA_POSTGRES_URL/DATABASE_URL or MULTICA_POSTGRES_HOST, MULTICA_POSTGRES_DB, MULTICA_POSTGRES_USER, and MULTICA_POSTGRES_PASSWORD." >&2
  exit 1
fi
